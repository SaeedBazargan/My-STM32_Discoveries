...
#define AI_HAR_MODEL_MODEL_SIGNATURE     "0xdc582ba86ee8a11fd06b698b258a4310"
...
#define AI_TOOLS_DATE_TIME   "2025-03-08T15:10:37+0330"
...
static ai_ptr g_har_model_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_har_model_weights_map[1] = AI_C_ARRAY_INIT;
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    ...
    375448, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 25616, 1, 1),
    25616, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_HAR_MODEL_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_HAR_MODEL_OUT_NUM, &dense_3_output),
  &conv2d_1_conv2d_layer, 0xc58b2752, NULL)
  ...
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ...
  err = ai_har_model_create(network, AI_HAR_MODEL_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_har_model_data_params_get(&params) != true) {
    err = ai_har_model_get_error(*network);
    return err;
  }
#if defined(AI_HAR_MODEL_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_HAR_MODEL_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_har_model_init(*network, &params) != true) {
    err = ai_har_model_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_buffer* ai_har_model_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_har_model_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
